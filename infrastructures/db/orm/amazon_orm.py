# -*- coding: utf-8 -*-
# @Author: yaccii
# @Description: 亚马逊结构化数据表（爬虫写入，本项目读取分析）
...
class SrcAmazonProductSnapshotsORM(TimestampMixin, Base):
    ...
    crawl_batch_no: Mapped[int] = mapped_column(BigInteger, nullable=False, comment="爬取批次号（用于关联一次任务）")
    ...
    __table_args__ = (
        UniqueConstraint("crawl_batch_no", "site", "asin", name="uq_ps_bsa"),
        Index("ix_ps_bs", "crawl_batch_no", "site"),
        Index("ix_ps_asin", "asin"),
    )
...
class SrcAmazonReviewsORM(TimestampMixin, Base):
    ...
    __table_args__ = (
        Index("ix_rv_bs", "crawl_batch_no", "site"),
        Index("ix_rv_asin", "asin"),
        Index("ix_rv_rid", "review_id"),
    )
...
class SrcAmazonKeywordMetricsORM(TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("crawl_batch_no", "site", "keyword", name="uq_kw_bsk"),
        Index("ix_kw_bs", "crawl_batch_no", "site"),
        Index("ix_kw_kw", "keyword"),
    )
