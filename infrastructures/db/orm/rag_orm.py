...
class DocumentORM(TimestampMixin, Base):
    ...
    uploader_user_id: Mapped[int] = mapped_column(Integer, nullable=False, comment="上传者user_id")
...
class IngestJobORM(TimestampMixin, Base):
    ...
    kb_space: Mapped[str] = mapped_column(String(64), nullable=False, comment="业务域")
    ...
    __table_args__ = (
        ...
        Index("ix_job_lock", "status", "locked_until"),
        Index("ix_job_doc", "document_id"),
    )
...
class ChunkORM(Base):
    ...
    locator: Mapped[dict | None] = mapped_column(JSON, nullable=True, comment="定位信息JSON(page/time_range/bbox等)")
    ...
    __table_args__ = (
        Index("ix_chunk_doc_ver", "document_id", "index_version"),
        Index("ix_chunk_space_ver", "kb_space", "index_version"),
        UniqueConstraint("document_id", "index_version", "chunk_index", name="uq_chunk_doc_ver_idx"),
    )
