...
class SpiderTaskService:
    def __init__(self) -> None:
        ...
        self.gateway = RedisGateway(
            redis_url=vconfig.spider_redis_url,
            list_key=vconfig.spider_redis_list_key,
            timeout_seconds=vconfig.spider_redis_timeout_seconds,
        )
    ...
    async def enqueue_task(self, db: AsyncSession, task: OpsSpiderTasksORM) -> None:
    ...
    async def get_task(self, db: AsyncSession, task_id: int) -> Optional[OpsSpiderTasksORM]:
        return await self.repo.get_by_id(db, task_id)

    async def get_by_key(self, db: AsyncSession, task_key: str) -> Optional[OpsSpiderTasksORM]:
